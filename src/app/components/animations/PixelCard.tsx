...
interface PixelCardProps {
  variant?: "default" | "blue" | "yellow" | "pink" | "green" | "gray";
  gap?: number;
  speed?: number;
  colors?: string;
  noFocus?: boolean;
...
export default function PixelCard({
  ...
  const doAnimate = (fnName: keyof Pixel) => {
    ...
    for (let i = 0; i < pixelsRef.current.length; i++) {
      ...
      // @ts-expect-error - We're dynamically calling pixel methods
      pixel[fnName]();
      if (!pixel.isIdle) {
        allIdle = false;
      }
  ...
  return (
    <div
      ref={containerRef}
      className={` h-[500px] w-full border border-[#F4F4F4] drop-shadow-lg relative overflow-hidden grid place-items-center aspect-[4/5] isolate transition-colors duration-200 ease-[cubic-bezier(0.5,1,0.89,1)] select-none ${className}`}
